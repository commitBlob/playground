...
export function renderPagination(options: GenerateOptions): string {
  const prevHref = (options.prevHref as string) || "";
  const nextHref = (options.nextHref as string) || "";

  let html = `<nav class="govuk-pagination" aria-label="Pagination">\n`;
